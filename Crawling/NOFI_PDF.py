...
# PDF 및 HWP 다운로드 함수
def download_files(num, url, folder="."):
    ...
    for i, file_link in enumerate(file_links, start=1):
        ...
        original_filename = content_disposition.split("filename=")[1].strip('"') if content_disposition else f"file{i}.pdf"
        original_filename = clean_filename(original_filename)
        
        # 파일명에 번호 추가
        filename, extension = os.path.splitext(original_filename)
        filename_with_number = f"{num}{abc}{extension}"
        
        # 파일 저장
        os.makedirs(folder, exist_ok=True)  # 폴더가 없으면 생성
        with open(os.path.join(folder, filename_with_number), 'wb') as file:
            for chunk in file_response.iter_content(chunk_size=1024):
                if chunk:
                    file.write(chunk)
        print(f"다운로드 완료: {filename_with_number}")
        abc += 1  # 카운터를 증가시킴
...
for link in full_url:
    num += 1
    download_files(num, link, folder="file_downloads")
